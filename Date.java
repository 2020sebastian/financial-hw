...
public class Date implements Comparable<Date> {
	...
	public Date(final int month, final int day, final int year) {
		if (!Date.isValid(month, day, year)) throw new RuntimeException("Invalid date");
		this.month = month;
		this.day   = day;
		this.year  = year;
	}
	...
	private static boolean isValid(final int m, final int d, final int y) {
		...
		if (d < 1 || d > Date.DAYS[m]) return false;
	...
	public Date next() {
		if (Date.isValid(month, day + 1, year))    return new Date(month, day + 1, year);
		else if (Date.isValid(month + 1, 1, year)) return new Date(month + 1, 1, year);
		else                                  return new Date(1, 1, year + 1);
	}


	// is this Date after b?
	public boolean isAfter(final Date b) {
		return compareTo(b) > 0;
	}

	// is this Date a before b?
	public boolean isBefore(final Date b) {
		return compareTo(b) < 0;
	}
